...
{
	using Data = Dictionary<int, BaseConfig>;
    using StrData = Dictionary<string, BaseConfig>;

	public class ConfigPool : Singleton<ConfigPool>
	{
		private const int jsonStartIndex = 4;
		Dictionary<Type, Data> DataPoolDic = new Dictionary<Type, Data>();
        Dictionary<Type, StrData> StrDataPoolDic = new Dictionary<Type, StrData>();
...
//#endif
         ...
         //   bLoadFromBundle = true;
            ...
            yield return waitForEndOfFrame;
            GangManager.Instance.InitBaseInfo();
            yield return waitForEndOfFrame;
            MissionSystemManager.Instance.Init();
            yield return waitForEndOfFrame;
            SkillManager.Instance.InitFabaoOriSkill();
            yield return waitForEndOfFrame;
            TmpGuideControl.Instance.InitGuideList();
            yield return waitForEndOfFrame;
            FriendManager.Instance.InitIntimacyInfo();
            yield return waitForEndOfFrame;
            RideManager.InitMissionGiveIDInfo();
            //标识build过了
            HasBuilt = true;
		}
        ...
        public bool LoadFormBundle<T>(AssetBundle AB, string JsonName) where T : BaseConfig, new()
        {
            ...
            do 
            {
                if (string.IsNullOrEmpty(JsonName))
                    break;
                TextAsset ta = AB.Load(JsonName) as TextAsset;
                if (null == ta)
                    break;
                try
                {
